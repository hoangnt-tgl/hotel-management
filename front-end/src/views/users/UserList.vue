              ...
              <div class="table-responsive datatable-vue user-list">
                ...
                >
                  ...
                  <template #cell(image)="field">
                    <img
                      height="50px"
                      :src="getImgUrl(field.item.user_image)"
                      width="50px"
                    />
                  </template>
                  ...
                  <template #cell(status) v-for="(item, index) in items">
                    {{ item.user_admin == 0 ? "Customer" : "Admin" }}
                    <!-- <feather
                      v-if="item.user_admin == 0"
                      :key="index"
                      type="circle"
                      stroke="#81BA01"
                      stroke-width="1"
...
export default {
  data() {
    return {
      ...
      tablefields: [
        ...
        { key: "user_fname", label: "First Name", sortable: true },
        { key: "user_lname", label: "Last Name", sortable: true },
  ...
  methods: {
    deleteSelected(item) {
      let objIndex = this.selected.findIndex((obj) => obj == item.user_id);
      if (objIndex > -1) {
        this.selected.splice(objIndex, 1);
      } else {
        this.selected.push(item.user_id);
      }
    },
    getImgUrl(path) {
    ...
    rowSelected(item) {
      this.selected = item.user_id;
      if (item[0]) {
        this.edit = this.edit !== item[0].user_id ? item[0].user_id : null;
      }
    },
        ...
        .then((response) => {
          for (var i = this.items.length - 1; i >= 0; i--) {
            for (var j = 0; j < this.selected.length; j++) {
              if (this.items[i] && this.items[i].user_id === this.selected[j]) {
